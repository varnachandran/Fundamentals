...
def max_duffel_bag_value(cake_tuples, weight_capacity):
    ...
    max_value_with_capacity = [0] * (weight_capacity+1)

    for cake_weight, value in cake_tuples:
        if cake_weight == 0 and value != 0:
            return float('inf')

        if cake_weight <= weight_capacity:
            max_value_with_capacity[cake_weight] = value

    for cake_weight, value in cake_tuples:
        for index in range(cake_weight, weight_capacity+1):
            max_value_with_capacity[index] = max(max_value_with_capacity[index], max_value_with_capacity[cake_weight] +
                                                 max_value_with_capacity[index-cake_weight] )
    return max_value_with_capacity[weight_capacity]

def max_duffel_bag_value_ic(cake_tuples, weight_capacity):
